...
Node *delete_at_starting_OF_head(Node *head){
  if (head == nullptr) return head; // edge case when LL is empty..
  Node *temp = head;
  head = head->next;
  delete temp; // to free the space of temp node...
  return head;
}
int main(){
  ...
  head = delete_at_starting_OF_head(head);
